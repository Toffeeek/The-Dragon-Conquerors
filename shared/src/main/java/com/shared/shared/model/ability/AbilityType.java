 ...
 *   <li><b>Cleric</b> has the strongest support kit and pays for Revive with
 *       low damage and a long ultimate cooldown.</li>
 *   <li><b>Soldier</b> is a durable close-range generalist: reliable weapon
 *       damage, a self-buff, and a heavy displacement ultimate.</li>
...
public enum AbilityType implements Ability {
    ...
    // ── Soldier: durable close-range pressure and self-sufficient utility ──
    SOLDIER_BASIC(new Spec("Steel Slash", "A disciplined sword strike at close range.")
        .owner(CharacterClass.SOLDIER).slot(AbilitySlot.PRIMARY)
        .damage(13).range(1.5f).target(TargetType.ENEMY)),

    SHIELD_BREAKER(new Spec("Shield Breaker", "A forceful strike that drives the target back.")
        .owner(CharacterClass.SOLDIER).slot(AbilitySlot.SECONDARY)
        .damage(18).mana(10).range(1.5f).push(1f).target(TargetType.ENEMY)),

    BATTLE_FOCUS(new Spec("Battle Focus", "Tightens your stance and raises Strength (+2).")
        .owner(CharacterClass.SOLDIER).slot(AbilitySlot.TERTIARY)
        .mana(12).cooldown(2)
        .boost(new StatBoost(StatType.STRENGTH, 2)).target(TargetType.SELF)),

    VANGUARD_ASSAULT(new Spec("Vanguard Assault",
        "Ultimate: a committed heavy strike that knocks the target backward.")
        .owner(CharacterClass.SOLDIER).slot(AbilitySlot.ULTIMATE)
        .damage(38).mana(24).range(2f).cooldown(4).push(1.5f).target(TargetType.ENEMY)),
